...
class Waypoint:
    ...
    @classmethod
    def from_record(cls, record: Dict[str, object]) -> 'Waypoint':
        pose_record = record.get('pose') or {}
        if not isinstance(pose_record, dict):
            raise ValueError('Waypoint pose must be an object.')
        return cls(
            name=str(record['name']),
            map_id=str(record['map_id']),
            pose=Pose2D(
                x=float(pose_record['x']),
                y=float(pose_record['y']),
