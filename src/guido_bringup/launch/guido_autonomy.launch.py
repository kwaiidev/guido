"""Bring up Guido's LiDAR navigation stack for ROS 2 Humble."""
...
def generate_launch_description():
    ...
    amcl_config = os.path.join(guido_bringup_share, 'config', 'amcl.yaml')
    nav_lifecycle_config = os.path.join(guido_bringup_share, 'config', 'nav_lifecycle_mgr.yaml')
    ...
    amcl = Node(
        ...
        remappings=[('/scan', '/navigation/filtered_scan')],
    )
    ...
    nav_lifecycle_manager = Node(
        ...
        output='screen',
        condition=IfCondition(use_amcl),
        parameters=[nav_lifecycle_config],
    )
